...
func TestStoreAppendAndLoad(t *testing.T) {
	...
	first := chat.UsageRecord{
		ConversationID: 42, Model: "deepseek-chat", PromptTokens: 11, CompletionTokens: 7, TotalTokens: 18,
	...
	if len(records) != 2 || records[0].ConversationID != first.ConversationID || records[0].Model != first.Model || records[0].PromptTokens != first.PromptTokens || records[0].CompletionTokens != first.CompletionTokens || records[0].TotalTokens != first.TotalTokens || records[0].PromptCacheHitTokens != first.PromptCacheHitTokens || records[0].PromptCacheMissTokens != first.PromptCacheMissTokens || !records[0].RequestedAt.Equal(first.RequestedAt) || records[1].Model != second.Model || !records[1].RequestedAt.Equal(second.RequestedAt) {
		t.Fatalf("registros inesperados: %#v", records)
	}
	if records[1].ConversationID != 0 || records[1].PromptTokens != 0 || records[1].CompletionTokens != 0 || records[1].TotalTokens != 0 || records[1].PromptCacheHitTokens != 0 || records[1].PromptCacheMissTokens != 0 {
...
func TestStoreLoadsRecordsWrittenBeforeConversationAssociation(t *testing.T) {
	path := filepath.Join(t.TempDir(), "usage.json")
	legacy := `[
  {
	...
	records, err := New(path).Load()
	if err != nil {
		t.Fatal(err)
	}
	if len(records) != 1 || records[0].ConversationID != 0 || records[0].TotalTokens != 12 {
		t.Fatalf("registro anterior incompatible: %#v", records)
	}
}
